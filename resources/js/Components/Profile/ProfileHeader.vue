    ...
    <div class="p-6">
      <div class="flex flex-col md:flex-row md:items-center gap-6">
        ...
        <div v-if="profile.profile_picture"
          class="relative h-28 w-28 rounded-full bg-white shadow-md overflow-hidden group border-4 border-white mx-auto md:mx-0">
          <img :src="profile.profile_picture ? profile.profile_picture : '/images/default-avatar.png'" :alt="fullName"
          ...
          <div v-if="isSameUser"
            class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-60 opacity-0 group-hover:opacity-100 transition-opacity">
            <label
              class="cursor-pointer text-white bg-[#193CB8] hover:bg-[#2748c6] px-3 py-1.5 rounded-md text-sm font-medium transition-colors">
        ...
        <div class="flex-1 text-center md:text-left">
          ...
          <div class="mt-4 flex items-center gap-3 justify-center md:justify-start">
            <a v-if="user.profile.linkedin" :href="user.profile.linkedin"
              class="bg-blue-50 hover:bg-blue-100 cursor-pointer p-2 flex rounded-full text-[#193CB8] transition-colors">
              <i class='bx bxl-linkedin text-xl'></i>
            </a>
            <a v-if="user.profile.github" :href="user.profile.github"
              class="bg-blue-50 hover:bg-blue-100 cursor-pointer p-2 flex rounded-full text-[#193CB8] transition-colors">
              <i class='bx bxl-github text-xl'></i>
            </a>
            <a v-if="user.profile.website" :href="user.profile.website"
              class="bg-blue-50 hover:bg-blue-100 cursor-pointer p-2 flex rounded-full text-[#193CB8] transition-colors">
