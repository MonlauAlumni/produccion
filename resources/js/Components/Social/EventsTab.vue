...
<template>
  <div class="py-8">
    ...
    <div v-else class="bg-white rounded-xl shadow-sm p-8 text-center">
      <div class="w-20 h-20 bg-[#193CB8]/10 rounded-full flex items-center justify-center mx-auto mb-4">
      ...
      <button @click="router.get('/eventos/nuevo')"
        class="px-6 py-3 bg-[#193CB8] text-white rounded-lg hover:bg-[#142d8c] transition-colors inline-flex items-center">
        <i class='bx bx-plus-circle mr-2'></i> Crear un evento
      </button>
    </div>

    <div v-if="events && events.length > 0" class="mt-8">
      <h3 class="text-xl font-bold text-gray-800 mb-4">Eventos destacados</h3>
