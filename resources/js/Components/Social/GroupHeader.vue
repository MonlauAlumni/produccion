    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <div class="p-6 pt-20 relative">
            <div class="absolute left-6 -top-12">
                <div class="relative">
                    <div class="w-24 h-24 rounded-xl overflow-hidden border-4 border-white bg-white shadow-md">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                ...
                <div class="mt-4 md:mt-0 flex items-center gap-3">
                    ...
                    <button v-if="isAdmin" @click="$emit('invite-members')"
                        class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors flex items-center cursor-pointer">
            ...
            <div v-if="group.tags && group.tags.length > 0" class="flex flex-wrap gap-2 mt-4">
                <span v-for="(tag, index) in group.tags" :key="index" @click="this.$inertia.visit(`/connect/search?q=${tag}`)"
                    class="px-2 py-1 bg-[#193CB8]/10 text-[#193CB8] rounded-full text-xs cursor-pointer hover:bg-[#193CB8]/20 transition-colors">
                    #{{ tag }}
                </span>
            </div>
            <div class="flex border-b border-gray-200 mt-6">
                <button v-for="tab in tabs" :key="tab.value" @click="handleTabChange(tab.value)"
                    ...
                    :class="activeTab === tab.value ? 'text-[#193CB8] border-b-2 border-[#193CB8]' : 'text-gray-500 hover:text-gray-700 cursor-pointer'">
                    {{ tab.label }}
                </button>
            </div>
        </div>
