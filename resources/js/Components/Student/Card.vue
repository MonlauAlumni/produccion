  ...
  <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition duration-300">
    ...
    <div class="p-4">
      <div class="text-center mb-3">
        ...
        <p v-if="alumni.profile?.job_title" class="text-sm text-gray-600">{{ alumni.profile.job_title }}</p>
      </div>
      
      <!-- Detalles adicionales -->
      <div class="space-y-2 mb-4">
        <div v-if="alumni.profile?.location" class="flex items-center text-gray-600">
          <i class='bx bx-map text-lg text-gray-500'></i>
          <span class="ml-2 text-sm">{{ alumni.profile.location }}</span>
        </div>
        
        <div v-if="alumni.profile?.graduation_year" class="flex items-center text-gray-600">
          <i class='bx bx-graduation text-lg text-gray-500'></i>
      ...
      <div class="flex items-center justify-center space-x-4 mb-4">
        <a 
          ...
          target="_blank" 
          class="text-gray-500 hover:text-[#0077B5] transition"
          aria-label="LinkedIn"
        >
        ...
        <a 
          ...
          target="_blank" 
          class="text-gray-500 hover:text-[#FF5722] transition"
