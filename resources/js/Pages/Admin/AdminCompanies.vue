    ...
    <AdminLayout title="Gestión de Empresas">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 w-full gap-2 mt-2">
            <InputWithIcon type="number" icon="id_card" placeholder="#" class="w-full" v-model="filters.id"
                @input="searchUsers" />
            <InputWithIcon type="text" icon="search" placeholder="Empresa" class="w-full" v-model="filters.company_name"
                @input="searchUsers" />
            <InputWithIcon type="number" icon="phone" placeholder="Teléfono" class="w-full" v-model="filters.company_phone"
                @input="searchUsers" />
                <InputWithIcon type="number" icon="wallet" placeholder="N. Fiscal" class="w-full" v-model="filters.fiscal_id"
                @input="searchUsers" />
                <InputWithIcon type="text" icon="mail" placeholder="Email" class="w-full" v-model="filters.mail"
                @input="searchUsers" />
        ...
        <span class="text-gray-400 mt-2 block text-right">
            Mostrando {{ (page.props.pagination.current_page - 1) * page.props.pagination.per_page + 1 }} - {{
                (page.props.pagination.current_page - 1) * page.props.pagination.per_page + page.props.companies.length }} de {{
                page.props.pagination.total }} usuarios
        </span>

        <Table :headers="['#', 'Empresa', 'Responsable','Teléfono', 'Número Fiscal', 'Correo', 'Acciones']">
            ...
            <tr v-else v-for="company in $page.props.companies" :key="company.id"
                ...
                <td class="py-2 px-4 text-blue-600 font-bold">{{ company.id }}</td>
                <td class="py-2 px-4">{{ company.company_name }} </td>
                <td class="py-2 px-4">{{ company.user?.name }} {{ company.user?.last_name_1 }} {{ company.user?.last_name_2 }}</td>
                <td class="py-2 px-4">{{ company.company_phone }}</td>
                <td class="py-2 px-4">{{ company.fiscal_id }}</td>
                <td class="py-2 px-4">{{ company.user?.email }}</td>
